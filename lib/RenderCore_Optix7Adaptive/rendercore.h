...
private:
	...
	CoreBuffer<float>* deviation = 0;				// buffer for per-pixel deviation estimation
	float devScale = 10.0f;							// scale for deviation, used to enforce sample budget
	CoreBuffer<float4>* frameAcc = 0;				// frame accumulator for adaptive sampling
	CoreBuffer<float4>* accumulator = 0;			// accumulator buffer for the path tracer
	CoreBuffer<float4>* features = 0;				// per-sample feature buffer for variance estimation
	...
	cudaEvent_t shadeStartAS[MAXPATHLENGTH], shadeEndAS[MAXPATHLENGTH];
...
{
public:
	inline void Init( RenderCore* core )
	{
		coreState = *core;
	}
	inline void Init( RenderCore* core, const ViewPyramid& pyramid )
	{
		coreState = *core;
		view = pyramid;
	}
