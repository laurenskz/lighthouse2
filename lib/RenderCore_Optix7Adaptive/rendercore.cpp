...
void InitCountersForASStage2();
void shade( const int pathCount, float4* accumulator, float4* features, const uint stride,
...
void finalizeASRender( const float4* frameAcc, float4* accumulator, const int w, const int h, const int samplesTaken, const int spp );
void finalizeFeatures( const float4* features, const int w, const int h, const int spp );
void estimateVariance( const float4* accumulator, const float4* features, float* deviation, const float scale, const int w, const int h, const int spp );
void generateAdaptiveSamples( const float* deviation, float4* pathStates, const uint R1, const uint shift,
...
void RenderCore::CreateOptixContext( int cc )
{
	...
	string ptx;
	if (NeedsRecompile( "../../lib/RenderCore_Optix7Adaptive/optix/", ".optix.turing.cu.ptx", ".optix.cu", "../../RenderSystem/common_settings.h", "../core_settings.h" ))
	{
		CUDATools::compileToPTX( ptx, TextFileRead( "../../lib/RenderCore_Optix7Adaptive/optix/.optix.cu" ).c_str(), "../../lib/RenderCore_Optix7Adaptive/optix", cc, 7 );
		if (cc / 10 == 7) TextFileWrite( ptx, "../../lib/RenderCore_Optix7Adaptive/optix/.optix.turing.cu.ptx" );
		else if (cc / 10 == 6) TextFileWrite( ptx, "../../lib/RenderCore_Optix7Adaptive/optix/.optix.pascal.cu.ptx" );
		else if (cc / 10 == 5) TextFileWrite( ptx, "../../lib/RenderCore_Optix7Adaptive/optix/.optix.maxwell.cu.ptx" );
		printf( "recompiled .optix.cu.\n" );
	}
	else
	{
		const char* file = NULL;
		if (cc / 10 == 7) file = "../../lib/RenderCore_Optix7Adaptive/optix/.optix.turing.cu.ptx";
		else if (cc / 10 == 6) file = "../../lib/RenderCore_Optix7Adaptive/optix/.optix.pascal.cu.ptx";
		else if (cc / 10 == 5) file = "../../lib/RenderCore_Optix7Adaptive/optix/.optix.maxwell.cu.ptx";
		FILE* f;
	#ifdef _MSC_VER
		fopen_s( &f, file, "rb" );
	#else
...
{
#ifdef _DEBUG
	printf( "Initializing Optix7Adaptive core - DEBUG build.\n" );
#else
	printf( "Initializing Optix7Adaptive core - RELEASE build.\n" );
#endif
...
void RenderCore::SetTarget( GLTexture* target, const uint spp )
{
	...
	if (reallocate)
	{
		...
		deviation = new CoreBuffer<float>( maxPixels, ON_DEVICE );
		frameAcc = new CoreBuffer<float4>( maxPixels * spp, ON_DEVICE );
		accumulator = new CoreBuffer<float4>( maxPixels, ON_DEVICE );
		features = new CoreBuffer<float4>( maxPixels * spp, ON_DEVICE );
		...
		params.frameAcc = frameAcc->DevPtr();
		params.deviation = deviation->DevPtr();
...
void RenderCore::RenderImpl( const ViewPyramid& view )
{
	...
	// sync params to device, TODO: copy all four in one copy?
	...
	for (int pathLength = 1; pathLength <= MAXPATHLENGTH; pathLength++)
	{
		...
		shade( pathCount, frameAcc->DevPtr(), features->DevPtr(), scrwidth * scrheight * scrspp,
	...
	if (counters.shadowRays > 0) CHK_OPTIX( optixLaunch( pipeline, 0, d_params[2], sizeof( Params ), &sbt, counters.shadowRays, 1, 1 ) );
	cudaEventRecord( shadowEnd );

#if 0

	// at 6spp + 60% AS, this is not (nearly) worth it...
	// abandoning this now until we have a better variance estimate.

	// estimate variance
	estimateVariance( frameAcc->DevPtr(), features->DevPtr(), deviation->DevPtr(), devScale, scrwidth, scrheight, scrspp );
	// wavefront loop for adaptive samples
	for (int pathLength = 1; pathLength <= MAXPATHLENGTH; pathLength++)
	{
		// generate / extend
		cudaEventRecord( traceStart[pathLength - 1] );
		if (pathLength == 1)
		{
			// generate and extend new eye rays based on variance estimate
			InitCountersForASStage2();
			generateAdaptiveSamples( deviation->DevPtr(), pathStateBuffer->DevPtr(), RandomUInt( camRNGseed ), shiftSeed,
				samplesTaken, blueNoise->DevPtr(), view, scrwidth, scrheight, scrspp );
			counterBuffer->CopyToHost(); // to obtain the number of paths generated by generateAdaptiveSamples
			counters = counterBuffer->HostPtr()[0];
			pathCount = counters.activePaths;
			const uint budget = (int)((scrwidth * scrheight * scrspp) * 0.25f);
			if (pathCount >= budget)
			{
				pathCount = counters.activePaths = budget;
				counterBuffer->CopyToDevice();
			}
			if (pathCount > 1.1f * budget) devScale *= 0.92f;			// big reduction
			else if (pathCount > 1.025f * budget) devScale *= 0.99f;	// small reduction
			else if (pathCount < 0.9f * budget) devScale *= 1.08f;		// big increase
			else if (pathCount < 0.975f * budget) devScale *= 1.01f;	// small increase
			printf( "adaptive samples: %i (%6.2f of budget), with devScale = %5.1f  ", pathCount, (float)pathCount * 100 / budget, devScale );
			if (pathCount > 0) CHK_OPTIX( optixLaunch( pipeline, 0, d_params[1], sizeof( Params ), &sbt, pathCount, 1, 1 ) );
		}
		else
		{
			// extend bounced paths
...
void RenderCore::WaitForRender()
{
	...
	devScale = renderThread->coreState.devScale;
...
void RenderCore::FinalizeRender()
{
	...
	finalizeASRender( frameAcc->DevPtr(), accumulator->DevPtr(), scrwidth, scrheight, samplesTaken, scrspp );
	// finalizeFeatures( features->DevPtr(), scrwidth, scrheight, scrspp );
#if 0
	if (samplesTaken > 2048)
	{
		accumulator->CopyToHost();
		const float scale = 1.0f / (samplesTaken / scrspp);
		for (int i = 0; i < scrwidth * scrheight; i++)
		{
			float4& p = accumulator->HostPtr()[i];
			p *= scale;
		}
		FILE* f = fopen( "reference.dat", "wb" );
		fwrite( accumulator->HostPtr(), sizeof( float4 ), scrwidth * scrheight, f );
		fclose( f );
		exit( 0 );
	}
#endif
	...
	if (!reference)
	{
		...
		reference = new float4[scrwidth * scrheight];
		fread( reference, sizeof( float4 ), scrwidth * scrheight, f );
		fclose( f );
	}
	accumulator->CopyToHost();
	const float scale = 1.0f / (samplesTaken / scrspp);
	...
	for (int i = 0; i < scrwidth * scrheight; i++)
	{
		float4 p1 = accumulator->HostPtr()[i];
		p1 *= scale;
		float4 p2 = reference[i];
		float v = sqr( p1.x - p2.x ) + sqr( p1.y - p2.y ) + sqr( p1.z - p2.z );
		total += v;
	}
	total /= (float)(scrwidth * scrheight);
	printf( "RMSE: %6.4f\n", sqrtf( total ) );
#endif
	...
	for (uint i = 0; i < MAXPATHLENGTH; i++)
	{
		...
		coreStats.shadeTime += CUDATools::Elapsed( renderThread->coreState.shadeStartAS[i], renderThread->coreState.shadeEndAS[i] );
	}
}
