...
void generateEyeRaysAdaptive( const float* deviation, Ray4* rayBuffer, float4* pathStateData, const uint R0, const uint pass, const uint* blueNoise,
...
void shade( const int pathCount, float4* accumulator, float4* features, const Ray4* extensionRays, const float4* extensionData,
	const Intersection* hits, Ray4* extensionRaysOut, float4* extensionDataOut, Ray4* shadowRays,
	float4* connectionT4, const uint R0, const uint shift, const uint* blueNoise, const int pass, const int2 probePos,
...
{
#ifdef _DEBUG
	printf( "Initializing PrimeAdaptive core - DEBUG build.\n" );
#else
	printf( "Initializing PrimeAdaptive core - RELEASE build.\n" );
#endif
...
void RenderCore::RenderImpl( const ViewPyramid& view )
{
	...
	const uint camR0 = RandomUInt( camRNGseed ), camR1 = RandomUInt( camRNGseed );
	...
	for (uint pathLength = 1; pathLength <= MAXPATHLENGTH; pathLength++)
	{
		...
		if (pathLength == 1) coreStats.traceTime0 = traceTime, coreStats.primaryRayCount = pathCount, coreStats.traceTimeX = 0;
		else if (pathLength == 2)  coreStats.traceTime1 = traceTime, coreStats.bounce1RayCount = pathCount, coreStats.deepRayCount = 0;
		else coreStats.traceTimeX += traceTime, coreStats.deepRayCount += pathCount;
		// shade
		cudaEventRecord( shadeStart[pathLength - 1] );
		shade( pathCount, frameAcc->DevPtr(), features->DevPtr(), extensionRayBuffer[inBuffer]->DevPtr(), extensionRayExBuffer[inBuffer]->DevPtr(),
			extensionHitBuffer->DevPtr(), extensionRayBuffer[outBuffer]->DevPtr(), extensionRayExBuffer[outBuffer]->DevPtr(),
			noDirectLightsInScene ? 0 : shadowRayBuffer->DevPtr(), shadowRayPotential->DevPtr(), samplesTaken * 7907 + pathLength * 91771, shiftSeed,
			blueNoise->DevPtr(), samplesTaken, probePos, pathLength, scrwidth, scrheight, view );
		...
		if (pathLength < MAXPATHLENGTH) // code below is not needed for the last path segment
		{
			...
			if ((counters.shadowRays + pathCount) >= maxShadowRays)
			{
				...
				finalizeConnections( counters.shadowRays, frameAcc->DevPtr(), shadowHitBuffer->DevPtr(), shadowRayPotential->DevPtr() );
	...
	// generate new eye rays based on variance estimate
	InitCountersForASStage2();
	generateEyeRaysAdaptive( deviation->DevPtr(), extensionRayBuffer[inBuffer]->DevPtr(), extensionRayExBuffer[inBuffer]->DevPtr(),
		camR1, samplesTaken, blueNoise->DevPtr(), view, GetScreenParams() );
	counterBuffer->CopyToHost();
	pathCount = counters.activePaths;
	float budget = scrwidth * scrheight * scrspp;
	if (pathCount > budget)
	{
	...
	printf( "adaptive samples: %i (%6.2f of budget), with devScale = %5.1f  ", pathCount, (float)pathCount * 100 / (scrwidth * scrheight * scrspp), devScale );
	// restart wavefront loop
	for (uint pathLength = 1; pathLength <= MAXPATHLENGTH; pathLength++)
	{
		float traceTime = TraceExtensionRays( pathCount );
		...
		cudaEventRecord( shadeStartAS[pathLength - 1] );
		shade( pathCount, frameAcc->DevPtr(), features->DevPtr(), extensionRayBuffer[inBuffer]->DevPtr(), extensionRayExBuffer[inBuffer]->DevPtr(),
			extensionHitBuffer->DevPtr(), extensionRayBuffer[outBuffer]->DevPtr(), extensionRayExBuffer[outBuffer]->DevPtr(),
			noDirectLightsInScene ? 0 : shadowRayBuffer->DevPtr(), shadowRayPotential->DevPtr(), samplesTaken * 59071 + pathLength * 31771, shiftSeed,
			blueNoise->DevPtr(), samplesTaken + scrspp, probePos, pathLength, scrwidth, scrheight, view );
		counterBuffer->CopyToHost();
		cudaEventRecord( shadeEndAS[pathLength - 1] );
