...
bool HandleInput( float frameTime )
{
	...
	{
		changed = true, camTime += frameTime * 0.1f;
	...
	if ( !keystates[GLFW_KEY_TAB] )
		tabDown = false;
	else if ( !tabDown ) // add a spline point
	{
...
void UpdateClouds( const float frameTime )
{
	...
	if ( c1pos < -40 )
		scale1 = ( c1pos + 50 ) * 0.1f;
	else if ( c1pos > 40 )
		scale1 = ( 50 - c1pos ) * 0.1f;
	if ( c2pos < -40 )
		scale2 = ( c2pos + 50 ) * 0.1f;
	else if ( c2pos > 40 )
		scale2 = ( 50 - c2pos ) * 0.1f;
	if ( c3pos < -40 )
		scale3 = ( c3pos + 50 ) * 0.1f;
	else if ( c3pos > 40 )
		scale3 = ( 50 - c3pos ) * 0.1f;
...
void Initialize()
{
	...
	// renderer = RenderAPI::CreateRenderAPI( "RenderCore_Optix7filter" );
	renderer = RenderAPI::CreateRenderAPI( "RenderCore_Custom" ); // RASTERIZER, your only option if not on NVidia
...
void DrawOverlay()
{
	...
	if ( delay < 1.8f )
	{
		sprintf( t, "Scene update: %6.2fms", systemStats.sceneUpdateTime * 1000 );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 1.6f )
	{
		sprintf( t, "Primary ray time: %6.2fms", coreStats.traceTime0 * 1000 );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 1.5f )
	{
		sprintf( t, "Secondary ray time: %6.2fms", coreStats.traceTime1 * 1000 );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 1.2f )
	{
		sprintf( t, "Shadow ray time: %6.2fms", coreStats.shadowTraceTime * 1000 );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 1.0f )
	{
		sprintf( t, "Overhead: %6.2fms", coreStats.frameOverhead * 1000 );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 0.9f )
	{
		sprintf( t, "# primary: %6ik (%6.1fM/s)", coreStats.primaryRayCount / 1000, coreStats.primaryRayCount / ( max( 1.0f, coreStats.traceTime0 * 1000000 ) ) );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 0.8f )
	{
		sprintf( t, "# secondary: %6ik (%6.1fM/s)", coreStats.bounce1RayCount / 1000, coreStats.bounce1RayCount / ( max( 1.0f, coreStats.traceTime1 * 1000000 ) ) );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 0.6f )
	{
		sprintf( t, "# deep rays: %6ik (%6.1fM/s)", coreStats.deepRayCount / 1000, coreStats.deepRayCount / ( max( 1.0f, coreStats.traceTimeX * 1000000 ) ) );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 0.4f )
	{
		sprintf( t, "# shadw rays: %6ik (%6.1fM/s)", coreStats.totalShadowRays / 1000, coreStats.totalShadowRays / ( max( 1.0f, coreStats.shadowTraceTime * 1000000 ) ) );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 0.3f )
	{
		sprintf( t, "camera position: %5.2f, %5.2f, %5.2f", camPos.x, camPos.y, camPos.z );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	if ( delay < 0.2f )
	{
		sprintf( t, "fdist: %5.2f", renderer->GetCamera()->focalDistance );
		tinyText->RenderR( t, x1, y1 );
		y1 += 13;
	}
	// present
...
void DemoState()
{
	...
	else
	{
		...
		if ( reportTimer > 0 )
		{
			bool show = true;
			if ( reportTimer < 0.5f )
				if ( (int)( reportTimer * 18.0f ) & 1 ) show = false;
...
void TitleState( bool results )
{
	...
	if ( results )
	{
		...
		smallText->Render( "PERFORMANCE", x + 1, y );
		y += 25;
		char t[1024];
		...
		smallText->Render( t, x, y );
		y += 30;
		smallText->Render( "primary rays", x, y );
		y += 25;
		...
		smallText->Render( t, x, y );
		y += 30;
		smallText->Render( "shadow rays", x, y );
		y += 25;
		...
		smallText->Render( "IMAGE QUALITY", x + 1, y );
		y += 25;
		float rmseSum = 0;
		for ( int i = 0; i <= 5; i++ )
		{
...
#endif
		y = lasty;
		smallText->Render( "Results have been saved to results.txt.", x, y );
	...
	else
	{
		...
		smallText->Render( "CREDITS", 15, y );
		y += 20;
		tinyText->Render( ""
						  "Medieval Fantasy Book"
						  " scene by Robert Jordan",
						  15, y );
		y += 15;
		tinyText->Render( "Music: "
						  "Movement Proposition"
						  ", by Kevin MacLeod",
						  15, y );
		y += 15;
		tinyText->Render( "Low poly bird animation by josluat91", 15, y );
		y += 15;
		tinyText->Render( "Additional models by Alok and Benjamin Todd", 15, y );
		y += 15;
		tinyText->Render( "Font: Source Code Pro by Google", 15, y );
		y += 15;
		tinyText->Render( "LH2 uses GLFW, Glad, TinyGLTF, TinyXML2, zlib, imgui, FreeImage and FreeType2.", 15, y );
		y += 20;
		tinyText->Render( "Lighthouse 2 engine and demo code by Jacco Bikker.", 15, y );
	}
	if ( results )
	{
		if ( frameTimer.elapsed() < 0.3f )
			textRenderer->Render( "Press [ENTER] to exit.", (int)( scrwidth * 0.355f ), scrheight - 90 );
		else if ( frameTimer.elapsed() > 0.6f )
			frameTimer.reset();
	}
	else
	{
		if ( frameTimer.elapsed() < 0.3f )
			textRenderer->Render( "Press [ENTER] to start.", (int)( scrwidth * 0.355f ), scrheight - 90 );
		else if ( frameTimer.elapsed() > 0.6f )
			frameTimer.reset();
	}
}
